...
def archive_landing_object(filename: str, dataset: str) -> bool:
    try:
        ...
        copy_source = {'Bucket': bucket_name, 'Key': f"pyspark_practise/landing/{dataset}/{filename}"}
        response = client.copy_object(Bucket = bucket_name, CopySource = copy_source, Key = f"pyspark_practise/archive/{dataset}/{filename}")
        if response['ResponseMetadata']['HTTPStatusCode'] == 200:
            client.delete_object(Bucket = bucket_name, Key = f"pyspark_practise/landing/{dataset}/{filename}")
            return True
        else:
            return False
